<template>
  <div>
    <div class="modal__title govuk-!-padding-2 govuk-heading-m">
      Handover checks consent form {{ typeOfEmail }}
    </div>
...
export default {
  ...
  methods: {
    ...
    async updateApplicationRecord(status) {
      await this.$store.dispatch('handoverChecks/updateStatus', {
    ...
    async send() {
      try {
        ...
        const response = await functions.httpsCallable('sendHandoverCheckRequests')({
