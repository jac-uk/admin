  ...
  <div>
    ...
    <div v-if="exercise.exercisePhoneNumber && exercise.emailSignatureName">
      ...
      >
        ...
        <div v-else-if="activeTab == 'declined'">
          <Banner
            :message="sendErrors"
            status="warning"
          />
          ...
          <Table
            key="declined"
            data-key="id"
            :data="assessmentsDeclinedRecords"
            :page-size="50"
            :columns="tableColumns"
            multi-select
            :selection.sync="selectedItems"
...
export default {
  ...
  computed: {
    tableColumns() {
      ...
      if (!this.onStaging && this.activeTab === 'notrequested') {
        return tableColumns;
      } else if (this.activeTab === 'cancelled' || this.activeTab === 'declined') {
        return [
          ...tableColumns,
          { title: 'Reason' },
        ];
